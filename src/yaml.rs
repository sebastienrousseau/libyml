...
use crate::YamlMappingStyleT::YamlAnyMappingStyle;
use crate::YamlSequenceStyleT::YamlAnySequenceStyle;
use core::ops::Deref;
...
/// The tag directive data (e.g., `%TAG !yaml! tag:yaml.org,2002:`).
#[derive(Copy, Clone, Debug)]
#[repr(C)]
#[non_exhaustive]
pub struct YamlTagDirectiveT {
    /// The tag handle (e.g., `!yaml!`).
    pub handle: *mut yaml_char_t,
    /// The tag prefix (e.g., `tag:yaml.org,2002:`).
...
/// The stream encoding (UTF-8, UTF-16, etc.).
...
pub enum YamlBreakT {
    ...
    YamlAnyBreak = 0,
    /// Use `\r` for line breaks (old Mac).
    YamlCrBreak = 1,
    /// Use `\n` for line breaks (Unix).
    YamlLnBreak = 2,
    /// Use `\r\n` for line breaks (DOS/Windows).
    YamlCrlnBreak = 3,
}
...
pub enum YamlScalarStyleT {
    ...
    /// The literal block scalar style (preserves whitespace).
...
impl Default for YamlScalarStyleT {
    fn default() -> Self {
        Self::YamlAnyScalarStyle
    }
}

/// Styles for YAML sequences (block or flow).
#[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Debug)]
#[repr(u32)]
#[non_exhaustive]
...
impl Default for YamlSequenceStyleT {
    fn default() -> Self {
        Self::YamlAnySequenceStyle
    }
}

/// Styles for YAML mappings (block or flow).
#[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Debug)]
#[repr(u32)]
#[non_exhaustive]
...
impl Default for YamlMappingStyleT {
    fn default() -> Self {
        Self::YamlAnyMappingStyle
    }
}

/// Possible token types generated by a lexical scan of the YAML stream.
#[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Debug)]
#[repr(u32)]
#[non_exhaustive]
pub enum YamlTokenTypeT {
    /// An empty token (no content).
    YamlNoToken = 0,
...
/// Represents one token in the token stream.
#[derive(Copy, Clone, Debug)]
#[repr(C)]
#[non_exhaustive]
pub struct YamlTokenT {
    /// The token type (scalar, anchor, etc.).
    pub type_: YamlTokenTypeT,
    /// The token data (unions for tag, alias, etc.).
...
impl Default for YamlEmitterT {
    fn default() -> Self {
        YamlEmitterT {
            ...
            output: UnnamedYamlEmitterTOutput::default(),
            ...
            best_width: 80,
            unicode: false,
            line_break: YamlBreakT::YamlAnyBreak,
            states: YamlStackT {
                start: ptr::null_mut(),
                end: ptr::null_mut(),
                top: ptr::null_mut(),
            },
            state: YamlEmitterStateT::YamlEmitStreamStartState,
            events: YamlQueueT::default(),
            indents: YamlStackT {
                start: ptr::null_mut(),
                end: ptr::null_mut(),
                top: ptr::null_mut(),
            },
            tag_directives: YamlStackT {
                start: ptr::null_mut(),
                end: ptr::null_mut(),
                top: ptr::null_mut(),
            },
            ...
            indention: true,
            open_ended: 0,
            anchor_data: UnnamedYamlEmitterTAnchorData::default(),
            tag_data: UnnamedYamlEmitterTTagData::default(),
            scalar_data: UnnamedYamlEmitterTScalarData::default(),
            opened: false,
            closed: false,
    ...
    fn default() -> Self {
        YamlEventT {
            ...
            data: UnnamedYamlEventTData {
                ...
                document_start: UnnamedYamlEventTdataDocumentStart {
                    version_directive: ptr::null_mut(),
                    tag_directives: UnnamedYamlEventTdataDocumentStartTagDirectives {
                        start: ptr::null_mut(),
                        end: ptr::null_mut(),
                    },
                    implicit: false,
                },
                document_end: UnnamedYamlEventTdataDocumentEnd { implicit: false },
                alias: UnnamedYamlEventTdataAlias { anchor: ptr::null_mut() },
                scalar: UnnamedYamlEventTdataScalar {
                    anchor: ptr::null_mut(),
                    tag: ptr::null_mut(),
                    value: ptr::null_mut(),
                    length: 0,
                ...
                sequence_start: UnnamedYamlEventTdataSequenceStart {
                    anchor: ptr::null_mut(),
                    tag: ptr::null_mut(),
                    implicit: false,
                    style: YamlAnySequenceStyle,
                },
                mapping_start: UnnamedYamlEventTdataMappingStart {
                    anchor: ptr::null_mut(),
                    tag: ptr::null_mut(),
                    implicit: false,
                    style: YamlAnyMappingStyle,
                },
            },
            start_mark: YamlMarkT::default(),
            end_mark: YamlMarkT::default(),
...
impl YamlEmitterT {
    /// Cleans up all dynamically allocated memory within `YamlEmitterT`.
    ///
    /// Frees (if allocated):
    /// - `anchor_data.anchor`
    /// - `tag_data.handle`, `tag_data.suffix`
    /// - `scalar_data.value`
    /// - Each `YamlTagDirectiveT` in `tag_directives`
    /// - Each `YamlEventT` in `events` (anchor/tag/value pointers, etc.)
    /// - The `anchors` array
    /// - The string buffer in `output` (if you allocated it)
    ///
    /// # Safety
    ///
    /// The caller must ensure that `self` is a valid pointer to an allocated
    /// `YamlDocumentT` that was allocated via `yaml_malloc` or `yaml_free`.
    /// No other references to this document may exist during cleanup.
    ...
    pub unsafe fn cleanup(&mut self) {
        ...
        if !self.anchor_data.anchor.is_null() {
            ...
            self.anchor_data.anchor = ptr::null_mut();
        }
        ...
        if !self.scalar_data.value.is_null() {
            ...
            self.scalar_data.value = ptr::null_mut();
        }
        ...
        self.tag_directives.top = ptr::null_mut();
        ...
        self.events.start = ptr::null_mut();
        self.events.end = ptr::null_mut();
        self.events.head = ptr::null_mut();
        self.events.tail = ptr::null_mut();
        ...
        if !self.output.string.buffer.is_null() {
            ...
            self.output.string.buffer = ptr::null_mut();
        }
        self.output.string.size = 0;
        self.output.string.size_written = ptr::null_mut();
        ...
        if !self.states.start.is_null() {
            ...
            self.states.start = ptr::null_mut();
            self.states.end = ptr::null_mut();
            self.states.top = ptr::null_mut();
        }
        if !self.indents.start.is_null() {
            yaml_free(self.indents.start as *mut libc::c_void);
            self.indents.start = ptr::null_mut();
            self.indents.end = ptr::null_mut();
            self.indents.top = ptr::null_mut();
        }
    ...
    /// Creates a new YAML emitter with default settings.
    ///
    ...
    /// Sets the `write_handler_data` field for the emitter.
    pub fn set_write_handler_data(&mut self, data: *mut libc::c_void) {
        self.write_handler_data = data;
    }
}
