...
mod tests {
    ...
    fn test_yaml_document_initialize_mixed_flags() {
        unsafe {
            ...
            assert_eq!(result, OK, "Initialization should handle mixed implicit and explicit flags");
            doc.cleanup();
        }
    }
    ...
    fn test_yaml_document_initialize_with_invalid_utf8_tag_directive() {
        unsafe {
            ...
            assert_eq!(result, FAIL, "Initialization should fail with invalid UTF-8 in tag directive");
        }
    }
    ...
    fn test_yaml_document_initialize_with_null_tag_directives() {
        unsafe {
            ...
            assert_eq!(result, OK, "Initialization should succeed with null tag directives");
            doc.cleanup();
        }
    }
    ...
    fn test_memory_allocation_failure() {
        unsafe {
            ...
            if result == FAIL {
                assert!(
                    true,
                    "Handled memory allocation failure correctly"
                );
            } else {
                doc.cleanup();
            }
